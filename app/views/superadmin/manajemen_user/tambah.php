...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Jika all_access, simpan semua plant & unit
    if ($all_access) {
    ...
    if ($nip && $username && $email && $password && $full_name && $role_id) {
        $check = $conn->prepare("SELECT user_id FROM users WHERE nip = ? OR email = ?");
        ...
        if ($check->rowCount() > 0) {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO users (nip,username,email,password,full_name,role_id,assigned_plants,assigned_units,all_access,status) VALUES (?,?,?,?,?,?,?,?,?,?)");
                $stmt->execute([$nip, $username, $email, $hash, $full_name, $role_id, $assigned_plants, $assigned_units, $all_access, $status]);
                set_flash('User berhasil ditambahkan!', 'success');
...
<head>
    ...
    <style>
        ...
        .checkbox-item label { margin:0;cursor:pointer;font-size:14px;text-transform:none;letter-spacing:normal;transition:opacity .3s }
        ...
        <div class="content">
            ...
            <div class="card">
                <form method="POST" action="<?= BASE_URL ?>">
                    <input type="hidden" name="page" value="superadmin.user-add">

                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                        <div class="form-group"><label>NIP *</label><input type="text" name="nip" class="form-control" required></div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="role_id" class="form-control" required id="roleSelect" onchange="toggleAssignSection()">
                                ...
                                <option value="<?= $r['role_id'] ?>"><?= ucfirst(htmlspecialchars($r['role_name'])) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <!-- Assignment Section (hanya admin & user) -->
                    <div id="assignmentSection" style="display:none;margin-top:24px">
                        ...
                        <label class="all-access-box" for="allAccessCheck">
                            <input type="checkbox" name="all_access" id="allAccessCheck" value="1" onchange="toggleAllAccess(this)">
                            <div>
                        ...
                        <!-- Plants -->
                        <div class="form-group" id="plantsSection">
                            ...
                            <div class="checkbox-group" id="plantsGroup">
                                ...
                                <div class="checkbox-item" id="plant-item-<?= $pl['plant_id'] ?>">
                                    <input type="checkbox" name="plants[]" value="<?= $pl['plant_id'] ?>"
                                           id="pl<?= $pl['plant_id'] ?>" onchange="updateUnitVisibility()">
                        ...
                        <!-- Units -->
                        <div class="form-group" id="unitsSection">
                            ...
                            <div class="checkbox-group" id="unitsGroup">
                                ...
                                <div class="checkbox-item unit-item" data-plant="<?= $un['plant_id'] ?>" style="display:none">
                                    <input type="checkbox" name="units[]" value="<?= $un['unit_id'] ?>" id="un<?= $un['unit_id'] ?>">
                                    <label for="un<?= $un['unit_id'] ?>"><?= htmlspecialchars($un['plant_desc'] . ' — ' . $un['unit_name']) ?></label>
                                </div>
                                <?php endforeach; ?>
                                <p id="unitPlaceholder" style="color:var(--text-secondary);font-size:13px;margin:0">Pilih plant terlebih dahulu</p>
                            </div>
                        </div>
                    </div>

                    <div style="display:flex;gap:10px;margin-top:30px">
                        <button type="submit" class="btn btn-primary"><i class="bi bi-floppy" style="margin-right:6px"></i>Simpan User</button>
...
function toggleAllAccess(cb) {
    ...
    // Dimkan semua plant & unit checkboxes
    ...
    if (isAll) {
        // Sembunyikan placeholder, tampilkan semua unit (tapi dimmed)
        document.getElementById('unitPlaceholder').style.display = 'none';
        document.querySelectorAll('.unit-item').forEach(el => el.style.display = 'flex');
    } else {
